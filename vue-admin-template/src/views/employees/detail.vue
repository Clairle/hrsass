...
export default {
  ...
  data() {
    return {
      ...
      userId: this.$route.params.id, // 从路由获取用户id
      formData: {
        username: '',
        password2: ''// 存储我们的修改值的临时字段
      },
      rules: {
  ...
  methods: {
    ...
    async saveUser() {
      try {
        ...
        await saveUserDetailById({ ...this.userInfo, password: this.userInfo.password2 }) // 对象中相同键，后面的属性替代前面的属性
        this.$message.success('保存成功')
        this.$refs.userForm.resetFields() // 重置表单校验
      } catch (error) {
        console.log(error)
      }
    }
